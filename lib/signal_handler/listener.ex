defmodule System.SignalHandler.Listener do
  ...
  def init(_args) do
    ...
    Task.start_link fn ->
      configured_handler_modules = Application.get_env(:signal_handler, :modules, [
        System.SignalHandler.GracefulShutdown
      ])

      configured_handler_modules |> Enum.each(fn(mod) ->
  ...
  def handle_call({:install, mod}, _from, %__MODULE__{handlers: handlers} = state) do
    signals = mod.__info__(:functions) |>
    ...
    new_registrations = signals |> Enum.map(fn(signal_name) ->
  ...
  def handle_call({:register, signal_name, new_handler}, _from, %__MODULE__{handlers: handlers} = state) do
    signal_code = state |> get_code(signal_name)
  ...
  defp build_signals_map do
    ...
    rt_signals = case sigrtmax do
      -1 -> []
      _  -> (sigrtmin..sigrtmax) |> Enum.map(fn(signum) ->
        {signum, realtime_signal_name(signum, sigrtmin, sigrtmax)}
      end)
    end |> Enum.into(%{})

    unknown_signals = (0..signum_bound) |> Enum.map(fn(i) ->
      {i, i}
    end) |> Enum.into(%{})
  ...
  defp normalize_handler(mod, sig_name) when is_atom(mod), do: fn ->
  ...
  defp get_code(%__MODULE__{by_name: by_name}, name) when is_atom(name), do: Map.fetch!(by_name, name)

  defp get_name(%__MODULE__{by_code: by_code}, code) when is_integer(code), do: Map.fetch!(by_code, code)
end
